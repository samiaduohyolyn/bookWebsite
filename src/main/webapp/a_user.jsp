...
<div class="box">
        ...
        <tr>
            <td valign="top">
                <table  width="965px" border="0" cellpadding="0" cellspacing="0" class="table1">
                    <tr>
                        <td colspan="3" align="center"><span style="font-size: 30px">用户管理</span></td>
                    </tr>
                    ...
                    <%
                        ...
                        sql = "select * from users limit " + (p - 1) * pagesize + "," + pagesize;
                    ...
                    <tr>
                        <td colspan="9" align="center">
                            <from action="a_user.jsp">
                                第<font style="color: red"><%=p %></font>页 共<font style="color: red"><%=pagecount %></font>页
                                <% if (p > 1) { %>
                                <a href="a_user.jsp?page=1"><button>第一页</button></a>
                                <a href="a_user.jsp?page=<%=p-1%>"><button>上一页</button></a>
                                <% }
                                    if (p < pagecount) { %>
                                <a href="a_user.jsp?page=<%=p+1%>"><button>下一页</button></a>
                                <a href="a_user.jsp?page=<%=pagecount %>"><button>最后一页</button></a>
                                <% } %>
                            </from>
                        </td>
                    </tr>
    ...
    <form method="post" action="a_user2.jsp">
        <div class="foot">
            <input name="name" class="m" type="text" placeholder="输入用户名称（例：admin）"><input  class="s" type="submit" value="查找">
        </div>
    </form>
</div>
</body>
