...
<ul>
    ...
    <li><a style="background: rgba(217, 125, 103, 0.73)" href="b_history.jsp">借阅纪录</a></li>
        ...
        <tr>
            <td valign="top">
                <table  width="965px" border="0" cellpadding="0" cellspacing="0" class="table1">
                    <tr>
                        <td colspan="9" align="center"><p style="font-size: 30px">本人借阅纪录</p></td>
                    </tr>
                    ...
                    <%
                        ...
                        String sql="select * from borrowbook where username='"+name+"'";
...
// 检查结果集是否为空
                        ...
                        sql = "select * from borrowbook where username='"+name+"' limit " + (p - 1) * pagesize + "," + pagesize;
                        System.out.println(sql);
                        rs = con.getRs(sql);
                        while (rs.next()) {
                    %>
                    <tr>
                        ...
                        <td><% if(Objects.equals(rs.getString(6), "否")) { %>
                            <a class="modbuttom" href="user ?id=<%= rs.getString(1)%>">还书</a>
                            <% } %>
                        </td>
                    </tr>
                    <% } %>
                    <tr>
                        <td colspan="9" align="center">
                                <% if (pagecount > 0) { %>
                                <from action="b_history.jsp">
                                    第<font style="color: #ff0000"><%=p %></font>页 共<font style="color: red"><%=pagecount %></font>页
                                    <% if (p > 1) { %>
                                    <a href="b_history.jsp?page=1"><button>第一页</button></a>
                                    <a href="b_history.jsp?page=<%=p-1%>"><button>上一页</button></a>
                                    <% }
                                        if (p < pagecount) { %>
                                    <a href="b_history.jsp?page=<%=p+1%>"><button>下一页</button></a>
                                    <a href="b_history.jsp?page=<%=pagecount %>"><button>最后一页</button></a>
                                    <% } %>
                                </from>
                                <% } %>
